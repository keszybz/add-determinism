...
impl Controller {
    ...
    pub fn send_path(&self, selected_handlers: u8, input_path: &Path) -> Result<()> {
        ...
        for input_path in &config.inputs {
            match handlers::process_file_or_dir(
                ...
                Some(&|selected_handlers, input_path| {
                    control.send_path(selected_handlers, input_path)
                }),
            ) {
                Err(err) => {
...
pub fn do_worker_work(config: options::Config) -> Result<()> {
    ...
    loop {
        ...
        debug!(
            "Will process {:?} (selected_handlers={})",
            input, selected_handlers
        );
