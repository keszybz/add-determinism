...
impl super::Processor for Javadoc {
    ...
    fn filter(&self, path: &Path) -> Result<bool> {
        Ok(
            path.extension().is_some_and(|x| x == "html"), // && path.to_str().is_some_and(|x| x.contains("/usr/share/javadoc/"))
        )
    }
    ...
    fn process(&self, input_path: &Path) -> Result<bool> {
        ...
        for line in input.lines() {
            ...
            if !after_header && (num >= HEADER_LINES_TO_CHECK || head_end_re.find(&line).is_some())
            {
                if !have_mod {
                    debug!(
                        "{}:{}: found nothing to replace until header end",
                        input_path.display(),
                        num
                    );
                }
...
mod tests {
    ...
    fn test_process_line() {
        ...
        assert_eq!(
            plu("<!-- Generated by javadoc (21) on Sat Mar 02 16:07:41 UTC 2024 -->").unwrap(),
        ...
        assert_eq!(
            plu(r#"<meta name="dc.created" content="2024-03-02">"#).unwrap(),
