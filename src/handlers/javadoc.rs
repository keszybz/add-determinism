...
fn process_line(config: &options::Config, line: &str) -> Result<Option<String>> {
...
pub fn process(config: &options::Config, input_path: &Path) -> Result<bool> {
    ...
    for line in input.lines() {
        ...
        let line2 = if !after_header { process_line(config, &line)? } else { None };
...
mod tests {
    ...
    const OPTS: options::Config = options::Config::empty(1704106800);
