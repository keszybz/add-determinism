...
fn format_ts(timestamp: zip::DateTime) -> [u8; 4] {
    ...
    [
        (tp & 0xFF).try_into().unwrap(),
        (tp >> 8).try_into().unwrap(),
        (dp & 0xFF).try_into().unwrap(),
        (dp >> 8).try_into().unwrap(),
    ]
}
        ...
        if let Some(dos_epoch) = self.dos_epoch {
            ...
            for i in 0..output.len() {
                ...
                match file.last_modified().to_time() {
                    ...
                    Ok(mtime) => {
                        let threshold = if file.name().ends_with(".clj") && unix_epoch_m_2 != unix_epoch {
                            &unix_epoch_m_2
                        } else {
                            &unix_epoch
                        };
